...
c
     ...
     +    write(*,'(i3,2f10.5,f7.1,a1,f12.5,5x,a1,f9.5)') 
   ...
   10 continue
      ...
      write(*,'(f12.5,"   Level 1-",i3)') xsdirdisctot(k0),nlev(Zix,Nix)
   ...
   20   continue    
     ...
     +      (elev(i),i=n2+1,n2+10)
          write(*,'(3x,10("    JP=",f4.1,a1)/)') 
     +      (jlev(i),cparity(plev(i)),i=n2+1,n2+10)
          do 50 iang=0,nangle
            write(*,'(f5.1,1p,10e12.5)') angle(iang),
     +        (dad(i,iang),i=n2+1,n2+10)
   50     continue    
   40   continue    
        if (nrest.gt.0) then
          write(*,'(/"Angle  ",10("Ex=",f6.3,"   "))') 
     +      (elev(i),i=nset+1,nset+nrest)
          write(*,'(3x,10("    JP=",f4.1,a1)/)') (jlev(i),
     +      cparity(plev(i)),i=nset+1,nset+nrest)
          do 60 iang=0,nangle
            write(*,'(f5.1,1p,10e12.5)') angle(iang),
...
c
     ...
     +  eoutgr(k0,3,2),Ggrcoll(3,2),betagr(3,2)
      ...
      if (flagddx) then
        ...
        do 110 iang=0,nanglecont   
          write(*,'(f5.1,1p,4e12.5)') anglecont(iang),
  ...
  110   continue
      ...
      if (flagspec) then
        ...
        write(*,'(" Energy   Total       GMR        GQR       LEOR",$)')
        write(*,'("       HEOR     Collective"/)')
        do 120 nen=ebegin(k0),eend(k0)
          write(*,'(f7.3,1p,6e11.4)') egrid(nen),xsgr(k0,nen),
