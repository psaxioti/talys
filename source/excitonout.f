...
c
      ...
      do 20 p=p0,maxpar
        h=p-p0       
        write(*,'(2i2,7f9.5)') p,h,(Qfactor(type,p),type=1,6)
   20 continue
c
...
c
      ...
      write(*,'(" p h",5x,7(a8,6x),"Total"/)') (parname(type),type=0,6)
      do 30 p=p0,maxpar
        h=p-p0
        write(*,'(2i2,1p,8e14.5)') p,h,
...
c
      ...
      do 50 p=p0,maxpar
        h=p-p0
        write(*,'(2i2,1p,e15.5)') p,h,lambdaplus(0,0,p,h)
   50 continue
      write(*,'(/"B. Damping widths (MeV)"/)')
      write(*,'(" p h     gammaplus"/)') 
      do 60 p=p0,maxpar
        h=p-p0
        write(*,'(2i2,1p,e15.5)') p,h,lambdaplus(0,0,p,h)*hbar
   60 continue
      write(*,'(/"C. Total widths (MeV)"/)')
      write(*,'(" p h     gammatot"/)') 
      do 70 p=p0,maxpar
        h=p-p0
        write(*,'(2i2,1p,e15.5)') p,h,
...
c
      ...
      do 80 p=p0,maxpar
        h=p-p0
        write(*,'(2i2,f10.5)') p,h,depletion(p,h)
   80 continue
c
...
c
      ...
      do 90 p=p0,maxpar
        h=p-p0
        write(*,'(2i2,1p,e15.5)') p,h,tauexc(p,h)
   90 continue
      return
      end
