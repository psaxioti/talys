...
c                                                                 
     ...
     +        bigr,bigb,xm,x1,sl,ce,term1,delta,beta,twxsec,xabs
c
...
c
      ...
      rela=ecmp+ecmt
      ecm=rela
      if (ecm.lt.0.1*rela) ecm=0.1*rela
     ...
     +  0.8/(1.+exp((250.-e)/75.))
      if (zproj.eq.1.and.aproj.eq.1) then
        ...
        if (iz.eq.12) then
          t1=40.
          const = 2.05 -3.0/(1.+exp((e-20)/10.))
        endif       
        if (iz.eq.14) then
          t1=40.
          const=2.05-1.75/(1.+exp((e-20.)/10.))
        endif       
        if (iz.eq.18) then
          t1=40.
          const=2.05-2.0/(1.+exp((e-20.)/10.))
        endif       
        if (iz.eq.20) then
          t1=40.
          const=2.05-1.0/(1.+exp((e-40.)/10.))
        endif       
      endif
	...
	if (iz.gt.82) const=const-real(iz)/(ia-iz)
        if (iz.ge.82) const=const-2.0/(1.+exp((e-20.)/20.))
        if (iz.le.20.and.iz.ge.10) 
     +    const=const-1.0/(1.+exp((e-20.)/10.))
      endif	
