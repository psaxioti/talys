...
c
      ...
      do 10 ns=1,maxmsd 
        write(*,'(i3,1p,3x,2(1x,e12.5))') ns,(msdstepint(type,ns),
     +    type=1,2)
   10 continue
     ...
     +    parsym(k0),parsym(type)
        ...
        do 30 nen=ebegin(type),eend(type)
          write(*,'(f7.3,1p,5e12.5)') egrid(nen),msdtot(type,nen),
...
c
     ...
     +      parsym(k0),parsym(type)
          write(*,'("for E-out= ",f7.3,/)') egrid(nen)
