...
c
      ...
      if (k0.eq.1) write(*,'("Total cross section   :",1p,e11.4," mb")')
     +  xstotinc
      write(*,'("Reaction cross section:",1p,e11.4," mb")') xsreacinc
      if (k0.eq.1) write(*,'("Elastic cross section :",1p,e11.4," mb")')
     +  xselasinc
c
...
c
      if (k0.eq.1.and.Einc.le.0.1) then
        ...
        write(*,'("potential scattering radius"/)')
        write(*,'("S0:",i4,f8.4," .10e-4 eV**-1/2")') Atarget,swave*1.e4
        write(*,'("P0:",i4,f8.4," .10e-4 eV**-1/2")') Atarget,pwave*1.e4
...
c
      ...
      write(*,'(a8," at ",f7.3," MeV"/)') parname(k0),Einc
c
...
c
      if (k0.eq.1.or.k0.eq.2.or.k0.eq.4.or.k0.eq.5) then
        ...
        do 10 l=0,lmaxinc
          write(*,'(i3,1p,3e13.5)') l,Tjlinc(-1,l),Tjlinc(1,l),Tlinc(l)
   10   continue
      endif
c
...
c
      if (k0.eq.3) then
        ...
        write(*,'("     Tav(L)"/)')
        do 20 l=0,lmaxinc
          write(*,'(i3,1p,4e13.5)') l,Tjlinc(-1,l),Tjlinc(0,l),
     +      Tjlinc(1,l),Tlinc(l)
   20   continue
      endif
c
...
c
      if (k0.eq.6) then
        write(*,'(" L     T(L)"/)')
        do 30 l=0,lmaxinc
          write(*,'(i3,1p,e13.5)') l,Tjlinc(0,l)
   30   continue
      endif
c
c 4. Photons                            
...
c
      if (flagang.and.k0.gt.0) then
        ...
        do 110 iang=0,nangle
          write(*,'(f5.1,1p,e16.5)') angle(iang),directad(k0,0,iang)
  110   continue                                           
      endif
      return
      end
